...
- `Button`: native button props; `variant="primary" | "secondary" | "outline" | "ghost"`; default primary. `size="default"` is 44px; primary also supports `size="hero"` at 48px. Default native type is `button`; explicitly choose `submit` in real forms. Supports native disabled and ref. No loading, destructive, icon-only, or link-button variants.
...
- `Badge`: required `variant="live" | "scheduled" | "draft" | "completed"`, native span attributes/ref, fixed matching text. Live includes a decorative pulse dot; reduced-motion disables animation. No extra status variants or implicit live-region announcements.
