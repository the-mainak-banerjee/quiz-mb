...
## Mandatory Primitive Reuse

For common UI elements, always use the existing shared primitives first.

Use:

- `Text` instead of raw `<p>`, `<span>`, `<h1>`, `<h2>` for styled text
...
1. Check the shared UI components.
2. Reuse an existing primitive if one exists.
3. Extend the existing primitive if needed.
4. Only create a new primitive if the concept is genuinely missing.

Semantic HTML should still be preserved through the primitive, e.g. `Text` may render `p`, `span`, or headings internally.
...
- Use `VisuallyHidden` component for icon only button
...
Always use the `cn` function from `utils.ts` file rather than using template literals for `classNames`
